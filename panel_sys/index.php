<?php 
	session_start();
	echo '<meta http-equiv="refresh" content="0;url=Modulos/principal">';
?>
<!DOCTYPE html>
<html lang="es">
  <head>
    ...
    <div class="container">
	  <form name="form1" method="post" action="" class="form-signin">
	  	...
	  	if(!empty($_POST['usu']) and !empty($_POST['con'])){ 
			...
			$pa=mysql_query("SELECT * FROM username, persona WHERE persona.cod='$usu' and username.usu='$usu' and username.con='$con'");	
			if($row=mysql_fetch_array($pa)){
				if($row['estado']=='s'){
					...
					if($row['tipo']=='a' or $row['tipo']=='u'){
						echo '<div class="alert alert-success" align="center"><strong>Bienvenido<br>'.$row['nom'].'<br>
